...
object Anagrams {
  ...
  def wordOccurrences(w: Word): Occurrences = w
    ...
    .toList
    .groupBy(identity)
    .map { case(k: Char, v: List[Char]) => (k, v.length) }
    .toList
    .sorted
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = {
    ...
    def allCombinations(occurrences: Occurrences): List[Occurrences] = occurrences match {
      case Nil => List(Nil)
      case (char, count) :: tail => for {
        f <- (0 to count).toList
        rest <- allCombinations(tail)
      } yield (char, f) :: rest
    }

    allCombinations(occurrences).map(
      _.filter {
        case (k, v) => v != 0
      }
    )
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    ...
    def subtractTerm(x: Map[Char, Int], y: (Char, Int)): Map[Char, Int] = {
      val (char, freq) = y
      x + (char -> (x(char) - freq))
    }

    (y.toMap foldLeft x.toMap)(subtractTerm).toList.filter(_._2 > 0).sorted
  }
    ...
    def occurrenceAnagrams(occ: Occurrences): List[Sentence] = {
      if (occ.isEmpty)
        ...
        for {
          ...
          rest <- occurrenceAnagrams(subtract(occ, comb))
        } yield word :: rest
    }
